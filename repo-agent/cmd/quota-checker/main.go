...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:   "quota-checker",
		Short: "Check GCP Generative AI API quota usage",
	}

	rootCmd.AddCommand(BuildQuotaCommand())
	rootCmd.AddCommand(BuildUsageCommand())
...
type PrintQuotaOptions struct {
	ProjectID string
}

func BuildQuotaCommand() *cobra.Command {
	var opt PrintQuotaOptions
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return RunPrintQuota(cmd.Context(), opt)
		},
	}

	cmd.Flags().StringVar(&opt.ProjectID, "project", opt.ProjectID, "Google Cloud Project ID")

	return cmd
}

func RunPrintQuota(ctx context.Context, opt PrintQuotaOptions) error {
...
type PrintUsageOptions struct {
	ProjectID string
}

func BuildUsageCommand() *cobra.Command {
	var opt PrintUsageOptions

	cmd := &cobra.Command{
		Use:   "usage",
		Short: "Check GCP Generative AI API usage",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) != 0 {
				return fmt.Errorf("command does not take positional arguments")
			...
			return RunPrintUsage(cmd.Context(), opt)
		},
	}
...
func RunPrintUsage(ctx context.Context, opt PrintUsageOptions) error {
	log := klog.FromContext(ctx)
	if opt.ProjectID == "" {
		return fmt.Errorf("--project is required")
	}
	...
	startTime := startOfDay.Add(-24 * time.Hour)
	endTime := nowInPacificTime.Add(-24 * time.Hour)
